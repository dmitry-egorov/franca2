...
#define bop_lambda(op) [](uint a, uint b) { return (uint)(a op b); }
        ...
        auto interpret_func_call (node&, context&) -> exec_result;
        ...
        auto interpret_store_u8    (node *, context &) -> exec_result;
    ...
    namespace interpretation {
        ...
        exec_result interpret_func_call(node& node, context& ctx) {
            ...
            if (fn_id ==   store_u8_id) return interpret_store_u8   (args_node_p, ctx);
        ...
        exec_result interpret_node(node& node, context& ctx) {
            ...
            switch (node.type) {
                case literal: {
                    if (is_uint_literal(node)) return to_res(node.uint_value);
                    if (is_str_literal (node)) return to_res(node.text);
                    if_ref(variable, find_var(storage, node.text)); else { dbg_fail_return {}; }
                    return to_res(variable.value);
                }
                case func: { return interpret_func_call(node, ctx); }
        ...
        exec_result interpret_store_u8(node * args, context & ctx) {
            using_exec_ctx;
        ...
        exec_result interpret_minus(node * args, context & ctx) {
            ...
            if (a_value.type == pt_u32); else { dbg_fail_return {}; }

            if_ref(b_node, a_node.next); else {
                return to_res(-a_r.value.i32);
            }
            ...
            if (b_value.type == pt_u32); else { dbg_fail_return {}; }

            return to_res(a_value.u32 - b_value.u32);
        }
        ...
        exec_result interpret_bop(node* args, uint (*f)(uint, uint), context & ctx) {
            ...
            if (a_value.type == pt_u32 && b_value.type == pt_u32); else {
                printf("Wrong binary op argument types: %u %u\n", (uint)a_value.type, (uint)b_value.type);
                dbg_fail_return {};
            }

            return to_res(f(a_value.u32, b_value.u32));
        }
        ...
        exec_result interpret_bop_a(node* args, uint (*f)(uint, uint), context& ctx) {
            ...
            if_ref(variable, find_var(storage, id)); else { dbg_fail_return {}; }
            if (variable.value.type == pt_u32); else { dbg_fail_return {}; }
            var r = interpret_node(init_node, ctx);
            if (r.value.type == pt_u32); else { dbg_fail_return {}; }
            if (r.is_returning) return r;
            variable.value = to_poly(f(variable.value.u32, r.value.u32));
            return r;
        }
        ...
        void push_value(dstring& storage, const poly_value& value) {
            switch (value.type) {
                case pt_u32: {
                    var text = make_string(*storage.arena, "%u", value.u32);
                    push(storage, text);
                    break;
                }
                case pt_str: {
                    push(storage, value.str);
                    break;
                }
                default: {
